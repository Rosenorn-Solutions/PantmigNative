  ...
  const onChangePassword = async () => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        show(res?.errorMessage || 'Kunne ikke opdatere adgangskoden', 'error');
      }
    } catch (e) {
  ...
  return (
    <View style={{ gap: 12 }}>
      <View style={local.card}>
        <Text style={local.sectionTitle}>Sikkerhed</Text>
        <View style={{ gap: 8 }}>
          <PressableButton
          ...
          {showPw ? (
            <View style={{ marginTop: 8 }}>
              <Text style={local.label}>Nuværende adgangskode</Text>
              <TextInput
                value={curPw1}
              ...
              <Text style={local.label}>Nuværende adgangskode (gentag)</Text>
              <TextInput
                value={curPw2}
              ...
              <Text style={local.label}>Ny adgangskode</Text>
              <TextInput
                value={newPw}
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                />
                <PressableButton title="Annullér" onPress={() => { setShowPw(false); }} color="#6b7280" iconName="xmark" />
              </View>
            </View>
          ) : null}
        </View>
      ...
      <View style={local.card}>
        <Text style={local.sectionTitle}>Konto</Text>
        <View style={{ gap: 8 }}>
          <PressableButton
          ...
          {showEmail ? (
            <View style={{ marginTop: 8 }}>
              <Text style={local.label}>Ny email</Text>
              <TextInput
                value={newEmail}
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                />
                <PressableButton title="Annullér" onPress={() => { setShowEmail(false); }} color="#6b7280" iconName="xmark" />
              </View>
            </View>
          ) : null}
        </View>
      ...
      <View style={local.card}>
        <Text style={local.sectionTitle}>Farezone</Text>
        <PressableButton
      ...
      <Modal visible={confirmDisable} transparent animationType="fade" onRequestClose={() => setConfirmDisable(false)}>
        <View style={modal.overlay}>
          <Pressable style={modal.backdrop} onPress={() => setConfirmDisable(false)} />
          <View style={modal.card}>
            <Text style={modal.title}>Deaktiver konto</Text>
            <Text style={modal.message}>Er du sikker?</Text>
            <View style={modal.buttonsRow}>
...
const modal = StyleSheet.create({
  overlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', zIndex: 50 },
  backdrop: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.45)' },
  card: { width: '92%', maxWidth: 420, backgroundColor: '#fff', borderRadius: 12, padding: 16, shadowColor: '#000', shadowOpacity: 0.15, shadowRadius: 12, shadowOffset: { width: 0, height: 8 }, elevation: 6 },
